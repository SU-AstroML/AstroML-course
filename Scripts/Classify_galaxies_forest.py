...
for n in Ns:
    ...
    decisions = rfc.predict_proba(data_test)[:,1]
...
#
imax = np.argmax(aucs)
print "Optimal tree number is ",Ns[imax]

rfc = RandomForestClassifier(Ns[imax]);
rfc.fit(data_train,labels_train)
print "trained"
print  [x for (y,x) in sorted(zip(rfc.feature_importances_,datanames))]
...
importances=  [x for (y,x) in sorted(zip(rfc.feature_importances_,range(len(datanames))))]
#
#
#
#dtree = clf.tree_
...
plt.savefig("Classify_trees.pdf")
#
